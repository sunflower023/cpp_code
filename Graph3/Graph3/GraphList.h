#pragma once
#include <iostream>
#include "linkQueue.h"//找代码，第三章讲过，链式队列
#include "Graph.h"
...
public:
    adjListGraph(int vSize, const TypeOfVer d[]); //构造
    void insert(TypeOfVer x, TypeOfVer y, TypeOfEdge w); //插入边
    void remove(TypeOfVer x, TypeOfVer y); //删除边
    bool exist(TypeOfVer x, TypeOfVer y) const; //判断两个结点是否邻接
    void dfs() const;  //深度优先
    void bfs() const; //广度优先
...
private:
    ...
    verNode* verList;  //邻接表  ，一维顶点结构体数组

    int find(TypeOfVer v) const {
        for (int i = 0; i < this->Vers; ++i)
            if (verList[i].ver == v) return i;
    }
    void dfs(int start, bool visited[]) const;  //深度优先内部函数
    verNode* clone() const;
